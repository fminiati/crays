...
namespace cr_transport{
  ...
  using dstfnc_t=vector<DistFunctNode>;
  ...
  void CRTransport::setup(const string a_file) {
    ...
    { // icm model
      ...
      for (auto& t : _icm._t) t/=_unit_time_sec;
      std::cout << "times: ";
      for (auto& t : _icm._t) std::cout << t << "  ";
    }
  ...
  void CRTransport::injection(std::vector<DistFunctNode>& a_f, const real_t a_dt) {
    ...
    vector<dstfnc_t> sinj(_num_threads,dstfnc_t(_num_nodes));
    ThreadUt<InjectionOp<GammaInjection>,dstfnc_t,const real_t> th_ut{_num_threads,
                                                                      _num_nodes};
    th_ut.launch_threads(injOp,sinj,a_dt);

    { auto t_beg=std::chrono::high_resolution_clock::now();
      // new distribution
      vector<DistFunctNode> fnew(_num_nodes,DistFunctNode{zero,zero});
      // add injection
  ...
  size_t CRTransport::advance(const real_t a_t, const real_t a_dt) {
    ...
    { // thread this time consuming spectrum calculation
      ...
      auto t_beg=std::chrono::high_resolution_clock::now();
      ThreadUt<const CRTranspOp,dstfnc_t,real_t> th_ut{_num_threads,f.size()};
      th_ut.launch_threads(crtOp,f,dts);
      auto t_end=std::chrono::high_resolution_clock::now();
      _timings["cr_transp"] += t_end-t_beg;

      // get min dt
      for (const auto dt : dts) dt_min = std::min(dt_min,dt);
    }
  ...
  void CRTransport::output(const real_t a_t, const unsigned a_step) {
    ...
    if (a_step == 0) {
      ...
      _t_output += _dt_output;
      {
        ...
        file << a_step << "   " << a_t << "   ";
        for (const auto& fp : _fc) file << fp.p << "   ";
        for (const auto& fp : _fc) file << fp.f << "   ";
        file << std::endl;
      ...
      if (_num_nu>0)
      {
        ...
        auto fp=[&f=std::as_const(fs),pmin=pmin*(one-w),pmax=pmax*(one+w),dlp](const real_t _p) {
                  ...
                  else if (_p>pmin) return (f.back().f  *exp(-_p/pmax));
                  else              return (f.front().f *exp(-pmin/_p));
                };
        ...
        { // define peak-p at peak of p^4*fs
          ...
          for (auto& n : nsy) n*=pow(ppk,2);
        }

        { // thread this time consuming spectrum calculation
          auto t_beg=std::chrono::high_resolution_clock::now();
          ThreadUt<const Synchrotron,vector<real_t>,const vector<real_t>> th_ut{_num_threads,
                                                                                _num_nu};
        ...
        file << a_step << "   " << a_t << "   ";
        for (auto n : nsy)       file << n*nc << "   ";
        for (const auto j : jsy) file << j << "   ";
